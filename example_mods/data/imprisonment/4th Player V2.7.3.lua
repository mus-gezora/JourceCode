...
characterExtraTrollion = 'characterExtraTrollion' -- Set this to what the sprite should be named.

noteNames = {
    '4th Player', -- Change this to whatever your note is named in path "custom_notetypes".
    '4th Player ALT', -- Change this to whatever your note is named in path "custom_notetypes". (ALT)
}
...
characterCamera = {
    690, -- X (Offset) for the camera to be targetting when it is extra character's turn.
    580 -- Y (Offset) for the camera to be targetting when it is extra character's turn.
}

characterScales = {
    0.3, -- Scale X of character
    0.3 -- Scale Y of character
}
...
function createCharacter(ExtraTrollion)
    ...
    makeAnimatedLuaSprite(ExtraTrollion, characterPath, characterPos[1], characterPos[2]);

    for i, v in pairs(animationData) do
        if stringTrim(v[4]) == '' or not v[4] then
            addAnimationByPrefix(ExtraTrollion, i, v[1], characterFPS, v[5]);
        elseif v[4] and not v[5] then -- else'em
            addAnimationByIndices(ExtraTrollion, i, v[1], v[4], characterFPS);
        elseif v[4] and v[5] then -- else'em
            addAnimationByIndicesLoop(ExtraTrollion, i, v[1], v[4], characterFPS);
        end

        addOffset(ExtraTrollion, i, v[2], v[3]);
    end

    setProperty(ExtraTrollion .. '.flipX', flipX);
    setProperty(ExtraTrollion .. '.antialiasing', antialiasing)
    scaleObject(ExtraTrollion, characterScales[1], characterScales[2]);

    setObjectOrder(ExtraTrollion, objectOrder);

    addLuaSprite(ExtraTrollion, false); -- Set this to true if you want it to be in front of other sprites. (You can also use object order to set where the sprite should overlap.)
...
function onCreate()
    ...
    createCharacter(characterExtraTrollion);
end
...
function onBeatHit()
    if isDanceIdle then
        if curBeat % danceEveryNumBeats == 0 and not skipDance and not stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') then
            dance()
            --holdTimer = 0
        end

    elseif not isDanceIdle then -- I did this differently than in Psych Engine. However, this works!
        if danced and not skipDance and curBeat % gfSpeed == 0 and not stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') then -- no stepCrochet?
            dance()
            --holdTimer = 0

        elseif not danced and not skipDance and curBeat % gfSpeed == 0 and not stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') then -- no stepCrochet?
            dance()
            --holdTimer = 0
        end
    end
...
function onUpdate(elapsed)
    if stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') then
    ...
    if flipX then 
        ...
        if holdTimer > stepCrochet * (0.0011 / getPropertyFromClass('flixel.FlxG', 'sound.music.pitch')) * singAnimationLength and stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') and not stringEndsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'miss') then
            dance()
        end
    end

    if stringEndsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'miss') and getProperty(characterExtraTrollion .. '.animation.curAnim.finished') then
        if stringStartsWith(version, '0.6') then playAnim(characterExtraTrollion, 'idle' .. idleSuffix, true, false, 10) elseif stringStartsWith(version, '0.7') then dance() playAnim(characterExtraTrollion, 'idle' .. idleSuffix, true, false, getProperty(characterExtraTrollion .. '.animation.curAnim.numFrames') - 1) end
    end

    if getProperty(characterExtraTrollion .. '.animation.curAnim.finished') and stringEndsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), '-loop') then
        playAnim(characterExtraTrollion, getProperty(characterExtraTrollion .. '.animation.curAnim.name') .. '-loop')
    end
    ...
    elseif specialAnim and getProperty(characterExtraTrollion .. '.animation.curAnim.finished') then
        specialAnim = false
        dance()
    end
end
...
function opponentNoteHit(membersIndex, noteData, noteType, isSustainNote)
        ...
        if noteType == noteNames[notes] then
            if isDanceIdle then
                ...
                if not noteNames[notes]:find('-') then
                    playAnim(characterExtraTrollion, singAnimations[noteData + 1] .. animSuffix, true);
                else
                    playAnim(characterExtraTrollion, tostring(stringSplit(noteNames[notes], '-')[2]), true);
                    specialAnim = true
                end
            else
                --playAnim(characterExtraTrollion, animNames[noteData + 2], true); -- uhhh is this needed?
                animSuffix = ''

                if getProperty(characterExtraTrollion .. '.animation.curAnim.name') == singAnimations[1] .. animSuffix then
                    danced = true
                elseif getProperty(characterExtraTrollion .. '.animation.curAnim.name') == singAnimations[2] .. animSuffix then
                    danced = false
                end

                if getProperty(characterExtraTrollion .. '.animation.curAnim.name') == singAnimations[3] .. animSuffix or getProperty(characterExtraTrollion .. '.animation.curAnim.name') == singAnimations[4] .. animSuffix then
                    danced = not danced
                end
            end
    ...
    if teamplaySettings.enabled then
        if teamplaySettings.sideDependent and (flipX or getProperty(characterExtraTrollion .. '.flipX')) then
...
function goodNoteHit(membersIndex, noteData, noteType, isSustainNote)
    ...
    --Sort this better?
    if teamplaySettings.enabled then
        if teamplaySettings.sideDependent and (not flipX or not getProperty(characterExtraTrollion .. '.flipX')) then
            if noteType == '' then -- Default Note (lol)
                animSuffix = ''
                playAnim(characterExtraTrollion, singAnimations[noteData + 1] .. animSuffix, true);
                holdTimer = 0
...
function noteMiss(membersIndex, noteData, noteType, isSustainNote)
    if noteType == noteNames[1] and haveMissAnimations then
        playAnim(characterExtraTrollion, singAnimations[noteData + 1] .. 'miss' .. animSuffix, true)
        --recalculateDanceIdle()
    end
end

function onTimerCompleted(ExtraTrollion, loops, loopsLeft)
    if ExtraTrollion == 'startCharacterTimer' then
        if isDanceIdle and luaSpriteExists(characterExtraTrollion) and loopsLeft % danceEveryNumBeats == 0 and getProperty(characterExtraTrollion .. '.animation.curAnim') and not stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') then
            dance()

        elseif not isDanceIdle and luaSpriteExists(characterExtraTrollion) and loopsLeft % gfSpeed == 0 and getProperty(characterExtraTrollion .. '.animation.curAnim') and not stringStartsWith(getProperty(characterExtraTrollion .. '.animation.curAnim.name'), 'sing') then -- gfSpeed and not round(gfSpeed * danceEveryNumBeats)? lol
            dance()
        end
    end
end
...
function onEvent(eventName, value1, value2)
    if eventName == characterExtraTrollion .. ' Idle ALT' or stringEndsWith(eventName, ' Idle ALT') then
        if value1 then
            idleSuffix = value1
        else
            idleSuffix = ''
    ...
    if eventName == characterExtraTrollion .. ' Camera Section' or stringEndsWith(eventName, ' Camera Section') then
        if value1 == '1' then
            extraSection = true
    ...
    if eventName == 'Play Animation' then -- could be an issue if the default character if not specified is bf or whatever.
        if value2 == characterExtraTrollion then
    ...
    if eventName == 'Extr. Char Change' then --this shit finna be real
        if value1 == characterExtraTrollion then
            ...
            removeLuaSprite(characterExtraTrollion, true)

            makeAnimatedLuaSprite(characterExtraTrollion, animationData.image, characterPos[1], characterPos[2]);

            for i = 1, #animationData.animations do
                if not animationData.animations[i].indices[1] then
                    addAnimationByPrefix(characterExtraTrollion, animationData.animations[i].anim, animationData.animations[i].name, animationData.animations[i].fps, animationData.animations[i].loop)
                elseif animationData.animations[i].indices[1] and not animationData.animations[i].loop then
                    addAnimationByIndices(characterExtraTrollion, animationData.animations[i].anim, animationData.animations[i].name, table.concat(animationData.animations[i].indices, ','), animationData.animations[i].fps);
                elseif animationData.animations[i].indices[1] and animationData.animations[i].loop then
                    addAnimationByIndicesLoop(characterExtraTrollion, animationData.animations[i].anim, animationData.animations[i].name, table.concat(animationData.animations[i].indices, ','), animationData.animations[i].fps);
                end
                ...
                addOffset(characterExtraTrollion, animationData.animations[i].anim, animationData.animations[i].offsets[1], animationData.animations[i].offsets[2])
            end


            setProperty(characterExtraTrollion .. '.antialiasing', not animationData.no_antialiasing)
            singAnimationLength = animationData.sing_duration
            characterCamera = animationData.camera_position
            scaleObject(characterExtraTrollion, animationData.scale, animationData.scale)
            addLuaSprite(characterExtraTrollion, true)
            if v2[2] and (tonumber(v2[2]) > 0 or stringTrim(v2[2]) ~= '') then objectOrder = v2[2] setObjectOrder(characterExtraTrollion, v2[2]) end
            dance()
        end
    end
end
...
function recalculateDanceIdle() -- Does settingCharacterUp & lastDance variable be needed for this? (Psych Engine does this but it seems to be fine without it? I don't know.)
    ...
    isDanceIdle = (getProperty(characterExtraTrollion .. '.animation.curAnim.name') == 'danceLeft' .. idleSuffix and getProperty(characterExtraTrollion .. '.animation.curAnim.name') == 'danceRight' .. idleSuffix)
    ...
    if not skipDance and not specialAnim then
        if not isDanceIdle then
            ...
            if danced then -- no stepCrochet?
                playAnim(characterExtraTrollion, 'danceRight' .. idleSuffix, true);
                holdTimer = 0
    
            elseif not danced then -- no stepCrochet?
                playAnim(characterExtraTrollion, 'danceLeft' .. idleSuffix, true);
                holdTimer = 0
            end

        elseif isDanceIdle then
            playAnim(characterExtraTrollion, 'idle' .. idleSuffix, false);
        end
    end
end
