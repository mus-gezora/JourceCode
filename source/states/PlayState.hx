...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['BOOOOOOO *throws tomatoes at u*', 0.4], //From 20% to 39%
		['You Suck! Prepare to be banned!', 0.5], //From 40% to 49%
		['skull', 0.6], //From 50% to 59%
		['Joekay...', 0.7], //From 60% to 69%
		['Okie Dokie', 0.8], //From 70% to 79%
		['It\'s Peam...', 0.9], //From 80% to 89%
		['Wowie Zowie!', 0.99], //From 90% to 99%
		['Those Who Know:', 1], //From 90% to 99%
		['So Amzing!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var imprisonment:String = 'Imprisonment';

	var composer:String;
	var cornerTxt:FlxText;
	public var songsByGezora:Array<String> = ['Injroe'];
	public var songsByStreetSign:Array<String> = ['Irresponsible', 'Parole', 'Hilarious'];
	var debugSong:String = 'Test';
    public function findComposer():String
		{
		if (songsByGezora.contains(SONG.song)) return "mus_gezora";
		if (songsByStreetSign.contains(SONG.song)) return "Street Sign";
		if (imprisonment.contains(SONG.song)) return "mus_gezora (ft. Street Sign)";
		if (debugSong.contains(SONG.song)) return "KawaiSprite and MtH";
		return "KawaiSprite";
		}
	public var maniaSongs:Array<String> = ['Injroe', 'Irresponsible', 'Parole'];
	...
	override public function create()
	{
		...
		cornerTxt = new FlxText(10, FlxG.height - 25, 0, composer + " - " + SONG.song + " (" + Difficulty.getString().toUpperCase() + ")", 74);
        cornerTxt.scrollFactor.set();
        cornerTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        cornerTxt.size = 18;
        cornerTxt.updateHitbox();
        cornerTxt.alpha = 1;
        cornerTxt.visible = !ClientPrefs.data.hideHud;
        cornerTxt.cameras = [camHUD];
		add(cornerTxt);
	...
	{
		trace(SONG.song.toLowerCase() + '_' + Difficulty.getFilePath().toLowerCase() + '_nomiss');
		if(startedCountdown) {
	...
	public dynamic function fullComboFunction()
	{
		...
		if(songMisses == 0)
		{
			...
			else if (goods > 0) ratingFC = '99FC';
			else if (sicks > 0) ratingFC = '100FC';
		}
		else {
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				if(Math.floor(Math.random() * 0) == 0 && SONG.song.toLowerCase() == 'imprisonment') 
				{
					FlxG.sound.music.stop();
					FlxTransitionableState.skipNextTransIn = true;
					MusicBeatState.switchState(new WhenKneesState());
				} else {
					vocals.stop();
					opponentVocals.stop();
					FlxG.sound.music.stop();
	...
	public function endSong()
	{
		...
		var songNoMiss:String = SONG.song.toLowerCase() + '_' + Difficulty.getFilePath().toLowerCase() + '_nomiss';
		checkForAchievement([weekNoMiss, songNoMiss, 'ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger']);
		#end
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.save.data.StoryCompleted = true;
					Mods.loadTopMod();
	...
	var gainHealth:Bool = false; // prevent health gain, *if* sustains are treated as a singular note
	function opponentNoteHit(note:Note):Void
	{
		...
		if(!note.isSustainNote && gainHealth == false)
        {
            switch(SONG.song.toLowerCase())
            {
                case 'injroe' | 'imprisonment' | 'irresponsible' | 'parole' | 'hilarious': {
                    if (health > 0.1) {
                        health -= 0.015;
                        if (health < 0.1) 
                        { 
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) //Common notes
		{
			...
			gainHealth = true; // prevent health gain, *if* sustains are treated as a singular note
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		for (name in achievesToCheck) {
			...
			if (name != WeekData.getWeekFileName() + '_nomiss' && name != SONG.song.toLowerCase() + '_' + Difficulty.getFilePath().toLowerCase() + '_nomiss') // common achievements
			{
				switch(name)
				{
					case 'ur_bad':
